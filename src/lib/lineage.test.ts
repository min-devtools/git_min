...
const tip = lineage([c("t1", ["base"], "wip", ["feature/B"]), ...history], "t1")!;
assert.deepStrictEqual(tip.merges, [], "feature/B was never merged");

// a commit whose fork point is off the loaded pages
const partial = lineage([c("x3", ["x2"], "tip", ["feature/C"]), c("x2", ["x1"], "b"), c("x1", ["gone"], "a")], "x2")!;
...
assert.strictEqual(lineage(history, null), null, "no selection → no card");
assert.strictEqual(lineage(history, "nope"), null, "unknown hash → no card");
